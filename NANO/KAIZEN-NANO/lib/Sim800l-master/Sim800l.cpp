...
void Sim800l::activateBearerProfile(){
  ...
  SIM.print (F(" AT+SAPBR=3,1,\"APN\",\"afrihost\" \r\n" ));_buffer=_readSerial(); // set apn  
...
void Sim800l::sendHTTP(String data ){
   SIM.print (F("AT+HTTPINIT\r\n "));
    SIM.print (F("AT+HTTPPARA=\"CID\",1"));
		String url = "http://ie-gtfs.up.ac.za/data/z-nano.php"+data;
		char at[url.length() + 42];
		sprintf(at, "AT+HTTPPARA=\"URL\",\"%s\"\r\n ", url.c_str());
		
		//Serial.println(at);
		 SIM.print (at);
		 SIM.print (F("AT+HTTPPARA=\"PROPORT\",\"80\"\r\n "));
		 SIM.print (F("AT+HTTPACTION=0\r\n "));
}
